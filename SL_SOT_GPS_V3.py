...
def plot_selected_routes_with_background(routes, background_image_path, destination_type, title_prefix="Fastest route to"):
    try:
        ...
        ax.set_aspect('equal')
        ax.imshow(img, extent=[1, 27, 1, 27])  # Set the extent of the image
        ax.plot(start[0], start[1], 'bo')
        ax.text(start[0], start[1], ' Start', verticalalignment='bottom', horizontalalignment='right')
        ax.set_xticks(range(1, 27))
        ax.set_xticklabels([chr(ord('A') + i) for i in range(26)])  # Label x-axis with letters A-Z
        ...
        for route in routes:
            ...
            if fastest_turn_original:
                ...
                ax.plot(end_loc[0], end_loc[1], 'rx')
                ax.text(end_loc[0], end_loc[1], f' {outpost_name} ({end_location_name})', verticalalignment='top', horizontalalignment='left')
                ...
                ax.plot([start[0], turn_x], [start[1], turn_y], line_style_turn)
                ax.plot([turn_x, end_loc[0]], [turn_y, end_loc[1]], line_style_final)
                ax.plot(turn_x, turn_y, color_turn_point)
                ax.text(turn_x, turn_y, f' Turn ({fastest_turn})', verticalalignment='bottom', horizontalalignment='right')
            else:
                # Plot the straight-line route
                ax.plot(end_loc[0], end_loc[1], 'rx')
                ax.text(end_loc[0], end_loc[1], f' {outpost_name} ({end_location_name})', verticalalignment='top', horizontalalignment='left')
                line_style_final = 'g-' if route == fastest_route else 'k-'
                ax.plot([start[0], end_loc[0]], [start[1], end_loc[1]], line_style_final)

        ax.set_xlabel('X Coordinate')
        ax.set_ylabel('Y Coordinate')
        ax.set_title(f"{title_prefix} {destination_type}")
